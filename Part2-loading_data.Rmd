...
- `read_delim`: Your general purpose flat file reader specifying 
...
Load the periodic table data stored in the `data` folder.
...
- 1. Excel files using the `read_excel()` function in the `readxl` package,
- 1. CSV files using the `read_csv()` and `read_delim()` functions in the `readr` package
- 1. data from proprietary statistical software using the `read_spss()`, `read_stata()`, and `read_sas()` functions in the `haven` package
